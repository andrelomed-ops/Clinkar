                ...
                {view === 'USERS' && (
                    <div className="space-y-8 animate-in fade-in duration-500">
                        <div className="bg-zinc-900 p-8 rounded-3xl border border-zinc-800 space-y-6">
                            <h3 className="text-xl font-black uppercase italic">Control de Accesos</h3>
                            <div className="flex gap-4">
                                <input value={userSearchQuery} onChange={e => setUserSearchQuery(e.target.value)} className="flex-1 h-14 bg-zinc-950 border border-zinc-800 rounded-xl px-4 text-sm font-bold" placeholder="Nombre o ID..." />
                                <button onClick={handleSearchUsers} className="h-14 px-8 bg-white text-black font-black rounded-xl text-xs uppercase">Buscar</button>
                            </div>
                        </div>
                        <div className="space-y-4">
                            {users.map(u => (
                                <div key={u.id} className="p-6 bg-zinc-900 border border-zinc-800 rounded-3xl flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <div className="h-12 w-12 bg-zinc-950 rounded-xl flex items-center justify-center border border-zinc-800 text-zinc-500"><Users className="h-6 w-6" /></div>
                                        <div><h4 className="font-black italic">{u.full_name}</h4><p className="text-[10px] font-bold text-zinc-500">{u.role} • {u.id}</p></div>
                                    </div>
                                    <div className="flex gap-2">
                                        <button onClick={() => handleUpdateUserRole(u.id, 'admin')} className="h-10 px-4 bg-indigo-600 text-white text-[9px] font-black rounded-lg">ADMIN</button>
                                        <button onClick={() => handleUpdateUserRole(u.id, 'inspector', null, u.location || 'CDMX')} className="h-10 px-4 bg-emerald-600 text-white text-[9px] font-black rounded-lg">MECÁNICO</button>
                                        <button onClick={() => handleUpdateUserRole(u.id, 'buyer')} className="h-10 px-4 bg-zinc-800 text-zinc-400 text-[9px] font-black rounded-lg">COMPRADOR</button>
                                    </div>
                                </div>
                            ))}
                        </div>
