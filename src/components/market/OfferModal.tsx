    ...
    return (
        <>
            ...
            {isOpen && mounted && createPortal(
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-xl animate-in fade-in duration-300">
                    <div className="bg-zinc-900/80 backdrop-blur-2xl border border-zinc-700/50 w-full max-w-lg rounded-[3rem] p-8 md:p-10 shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] space-y-8 animate-in zoom-in-95 duration-300 relative max-h-[90vh] overflow-y-auto custom-scrollbar">
                        <button
                            onClick={() => setIsOpen(false)}
                            className="absolute top-8 right-8 h-10 w-10 rounded-full bg-zinc-800/50 text-zinc-400 hover:text-white hover:bg-zinc-700 transition-all flex items-center justify-center z-10 border border-zinc-700/50 backdrop-blur-md"
                        >
                            ✕
                        </button>

                        <div className="space-y-3">
                            <div className="inline-flex px-3 py-1 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded-full text-[10px] font-black uppercase tracking-[0.2em] italic">
                        ...
                        {/* Reliability Score */}
                        <div className="bg-zinc-950/40 rounded-3xl border border-zinc-800/50 p-1">
                        ...
                        <div className="bg-zinc-950/60 rounded-[2rem] p-8 border border-zinc-800/50 space-y-5">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="relative group">
                                <span className="absolute left-8 top-1/2 -translate-y-1/2 text-3xl font-black text-zinc-600 group-focus-within:text-amber-500 transition-colors">$</span>
                                <input
                                    type="number"
                                    value={offerAmount}
                                    onChange={(e) => setOfferAmount(Number(e.target.value))}
                                    className={cn(
                                        "w-full h-24 rounded-[2rem] bg-zinc-950/50 border-2 pl-16 pr-8 text-4xl font-black focus:ring-0 transition-all border-zinc-800/50 focus:border-amber-500 text-white selection:bg-amber-500/30 shadow-inner backdrop-blur-md"
                                    )}
                                />
                                <div className="absolute right-8 top-1/2 -translate-y-1/2 px-3 py-1 bg-zinc-800/80 rounded-lg text-[10px] font-black text-zinc-400">MXN</div>
                            </div>
                        </div>

                        {/* Cost Breakdown & Fee */}
                        <div className="bg-amber-500/5 rounded-[2.5rem] p-8 space-y-4 border border-amber-500/10 shadow-inner">
                            ...
                            <div className="pt-5 border-t border-zinc-800/50 flex justify-between items-center">
                                ...
                                <span className="text-3xl font-black text-white italic tracking-tighter">${offerAmount.toLocaleString()}</span>
                            </div>
                            <div className="flex items-center gap-2 text-amber-500 bg-amber-500/10 rounded-xl py-3 justify-center border border-amber-500/20">
                                <Clock className="h-4 w-4" />
                                <span className="text-[10px] font-black uppercase tracking-widest">Oferta válida por 24 horas</span>
                            </div>
                        </div>

                        <div className="p-6 bg-zinc-950/40 border border-zinc-800/50 rounded-[2rem] space-y-3">
                        ...
                        <div className="bg-blue-600/5 border border-blue-600/10 rounded-[2rem] p-8 space-y-3">
                            <h4 className="text-blue-400 font-black text-xs flex items-center gap-2 uppercase tracking-[0.2em] italic">
                                <CheckCircle2 className="h-4 w-4" />
                                Justificación del Valor
                            </h4>
                            <p className="text-zinc-400 text-[11px] leading-relaxed font-medium">
                                El monto de <span className="text-white font-bold">${offerAmount.toLocaleString()} MXN</span> es coherente con el dictamen de 150 puntos, asegurando una transacción justa.
                            </p>
                        </div>
                        ...
                        <button
                            onClick={handleOffer}
                            disabled={!isValid}
                            className="w-full h-20 rounded-[2rem] bg-amber-500 text-black font-black text-xl shadow-2xl shadow-amber-500/20 hover:bg-amber-400 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 disabled:grayscale uppercase italic tracking-tighter"
                        >
